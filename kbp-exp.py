...
if __name__ == '__main__':
    ...
    parser.add_argument( '--language', type = str, default = 'eng', choices = [ 'eng', 'cmn', 'spa' ] )
    ...
    numericizer2 = chinese_word_vocab( config.word_embedding + '-word.wordlist' )
    ...
    human = batch_constructor( imap( lambda x: x[:4], LoadED( config.data_path + '/cmn-train-parsed', 'cmn' ) ),
                               numericizer1, numericizer2, gazetteer = kbp_gazetteer, 
                               alpha = config.word_alpha, window = config.n_window, 
                               n_label_type = config.n_label_type, language = 'cmn' )
    logger.info( 'human: ' + str(human) )
    
    valid = batch_constructor( imap( lambda x: x[:4], LoadED( config.data_path + '/cmn-eval-parsed', 'cmn' ) ), 
                               numericizer1, numericizer2, gazetteer = kbp_gazetteer, 
                               alpha = config.word_alpha, window = config.n_window, 
                               n_label_type = config.n_label_type, language = 'cmn' )
    logger.info( 'valid: ' + str(valid) )
    
    test  = batch_constructor( imap( lambda x: x[:4], LoadED( config.data_path + '/cmn-train-parsed', 'cmn' ) ),
                               numericizer1, numericizer2, gazetteer = kbp_gazetteer, 
                               alpha = config.word_alpha, window = config.n_window, 
                               n_label_type = config.n_label_type, language = 'cmn' )
    ...
    for n_epoch in xrange( config.max_iter ):
        ...
        valid_predicted = open( 'kbp-result/kbp-valid.predicted', 'wb' )
        test_predicted = open( 'kbp-result/kbp-test.predicted', 'wb' )

        #############################################
        ########## go through training set ##########
        #############################################

        assert not config.enable_distant_supervision
        ...
        #     (n_epoch + 1) % min(16, config.max_iter / 16) == 0:
            ...
            if n_epoch >= config.max_iter / 2:
                pp = [ p for p in PredictionParser( # KBP2015(  data_path + '/ed-eng-eval' ), 
                                                    imap( lambda x: x[:4], LoadED( config.data_path + '/eng-eval-parsed' ) ),
                                                    'kbp-result/kbp-valid.predicted', config.n_window,
                                                    n_label_type = config.n_label_type ) ]
            ...
            precision, recall, f1, info = evaluation( PredictionParser( # KBP2015(  data_path + '/ed-eng-eval' ),
                                                                      imap( lambda x: x[:4], LoadED( config.data_path + '/cmn-eval-parsed', 'cmn' ) ),
                                                                      'kbp-result/kbp-valid.predicted', config.n_window,
                                                                      n_label_type = config.n_label_type ), 
            ...
            precision, recall, f1, info = evaluation( PredictionParser( # KBP2015(  data_path + '/ed-eng-train' ),
                                                                      imap( lambda x: x[:4], LoadED( config.data_path + '/cmn-train-parsed', 'cmn' ) ),
                                                                      'kbp-result/kbp-test.predicted', config.n_window,
                                                                      n_label_type = config.n_label_type ), 
                                                      best_threshold, best_algorithm, True,
                                                      n_label_type = config.n_label_type )
            logger.info( '%s\n%s' % ('test', info) ) 
    ...
    logger.info( 'results are written in kbp-result/kpb-{valid,test}.predicted' )
...
# numericizer1 = chinese_word_vocab( 'word2vec/wiki-cmn-char.wordlist' )
# numericizer2 = chinese_word_vocab( 'word2vec/wiki-cmn-word.wordlist' )

# data = list( imap( lambda x: x[:4], LoadED( 'processed-data/cmn-train-parsed', 'cmn' ) ) )
...
# train = batch_constructor( data,
#                            numericizer1, numericizer2, gazetteer = None, 
#                            alpha = 0.5, window = 7, n_label_type = 11,
#                            language = 'cmn' )
# print train

# for i, example in enumerate( train.mini_batch( 256, False, 1, 1, 4 ) ):
#   print 'hopeless %d' % i
